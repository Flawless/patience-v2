...
output "token_commands" {
  ...
  value = { for k, v in kubernetes_service_account.accounts : k => 
    "kubectl create token ${v.metadata[0].name} -n ${v.metadata[0].namespace}"
  }
}
...
output "export_commands" {
  ...
  value = { for k, v in kubernetes_service_account.accounts : k => 
    "export TF_VAR_k8s_token=$(kubectl create token ${v.metadata[0].name} -n ${v.metadata[0].namespace})"
  }
}
